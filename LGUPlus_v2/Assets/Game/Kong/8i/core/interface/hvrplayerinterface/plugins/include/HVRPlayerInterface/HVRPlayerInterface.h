...
API_INTERFACE void	Interface_ClearCache();

static const int INTERFACE_LOG_TYPE_ALL = 0;
static const int INTERFACE_LOG_TYPE_DEBUG = 0;
static const int INTERFACE_LOG_TYPE_INFO = 1;
static const int INTERFACE_LOG_TYPE_WARNING = 2;
static const int INTERFACE_LOG_TYPE_ERROR = 3;
static const int INTERFACE_LOG_TYPE_NONE = 4;
...
API_INTERFACE size_t Interface_GetStatCount();
API_INTERFACE bool Interface_GetStatName(size_t idx, char* name, int nameSize);
API_INTERFACE StatsData Interface_GetStatValue(const char* name);

//-----------------------------------------------------------------------------
// Player Functions

static const int RENDERER_TYPE_OPENGL = 0;
static const int RENDERER_TYPE_DIRECT3D11 = 1;
static const int RENDERER_TYPE_GNMX = 2;
static const int RENDERER_TYPE_METAL = 3;
...
static const int ASSET_STATE_INITIALISING	= 1 << 0;
static const int ASSET_STATE_PLAYING		= 1 << 1;
static const int ASSET_STATE_SEEKING		= 1 << 2;
static const int ASSET_STATE_CACHING		= 1 << 3;
static const int ASSET_STATE_OFFLINE		= 1 << 4;
static const int ASSET_STATE_FULLY_CACHED	= 1 << 5;
static const int ASSET_STATE_INVALID		= 1 << 6;
static const int ASSET_STATE_AVAILABLE		= 1 << 7;
...
API_INTERFACE float	Asset_GetActualTime(HVRID asset);
API_INTERFACE float	Asset_GetDecodeTime(HVRID asset);
API_INTERFACE float	Asset_GetDuration(HVRID asset);
API_INTERFACE bool Asset_GetCodec(HVRID asset, char* codec, size_t size);
...
API_INTERFACE float Asset_GetVoxelSize(HVRID asset);
API_INTERFACE uint32_t Asset_GetCurrentBandwidth(HVRID asset);
API_INTERFACE bool Asset_GetAvailabilityStartTime(HVRID asset, char* value, size_t valueSize);

static const int ASSET_TYPE_STATIC		= 1 << 0;
static const int ASSET_TYPE_DYNAMIC		= 1 << 1;
API_INTERFACE int Asset_GetType(HVRID asset);
...
static const int COLOUR_SPACE_GAMMA = 0;
static const int COLOUR_SPACE_LINEAR = 1;
