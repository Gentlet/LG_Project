        ...
        public static void OnPostprocessBuild(BuildTarget target, string buildPath)
        {
            ...
            else
            {
                ...
                if (buildTarget == BuildTarget.iOS)
                {
                    ...
                    EditorSharedResources esr = EditorSharedResources.instance;

                    if (esr == null)
                    {
                        string[] guids = AssetDatabase.FindAssets("t:" + typeof(EditorSharedResources).Name);

                        foreach (string s in guids)
                        {
                            string path = AssetDatabase.GUIDToAssetPath(s);
                            esr = AssetDatabase.LoadAssetAtPath(path, typeof(EditorSharedResources)) as EditorSharedResources;
                        }
                    }

                    if (esr != null &&
                        esr.plugin_HvrPlayerInterfaceFramework_iOS != null)
                    {
                        string pluginAssetPath = AssetDatabase.GetAssetPath(esr.plugin_HvrPlayerInterfaceFramework_iOS);
                    ...
                    {
                        Debug.LogWarning("Unable to find HVRPlayerInterface.framework. It may needed to be added manually to the XCode Project.");
                    }
