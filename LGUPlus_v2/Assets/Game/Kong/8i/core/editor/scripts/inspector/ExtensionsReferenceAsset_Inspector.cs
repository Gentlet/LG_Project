...
namespace HVR.Editor
{
    ...
    public class ExtensionsReferenceAsset_Inspector : UnityEditor.Editor
    {
        ...
        private void DrawElement(Rect rect, int index, bool active, bool focused)
        {
            ...
            float yOffset = 0;
            reference.platform = (RuntimePlatform)EditorGUI.EnumPopup(new Rect(rect.x, rect.y + yOffset, rect.width, EditorGUIUtility.singleLineHeight), "Platform", reference.platform);
            yOffset += EditorGUIUtility.singleLineHeight;
            reference.guid = HvrEditorGUI.DrawObjectSlot(new Rect(rect.x, rect.y + yOffset, rect.width, EditorGUIUtility.singleLineHeight), "Data", reference.guid);
            yOffset += EditorGUIUtility.singleLineHeight;
