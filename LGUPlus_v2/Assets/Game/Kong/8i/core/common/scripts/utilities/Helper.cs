...
namespace HVR
{
    public class Helper
    {
        ...
        public const float AUDIO_TO_VIDEO_FIX = 30.0f / 29.8f;
        public const float VIDEO_TO_AUDIO_FIX = 29.8f / 30.0f;

        static HashSet<GCHandle> allGCHandles = new HashSet<GCHandle>();
        ...
        public static void RegisterGCHandle(GCHandle handle)
        {
            allGCHandles.Add(handle);
        }

        public static void DeregisterGCHandle(GCHandle handle)
        {
            allGCHandles.Remove(handle);
        }

        public static bool IsGCHandleRegistered(GCHandle handle)
        {
            return allGCHandles.Contains(handle);
        }
        ...
        public static T GCHandleToObject<T>(GCHandle handle)
        {
            try
            {
                if (!IsGCHandleRegistered(handle))
                {
                    return default(T);
                }

                if (handle.Target.GetType() == typeof(T) ||
                    handle.Target.GetType().IsSubclassOf(typeof(T)))
                {
        ...
        public static void GetUnityVersion(out int majorVersion, out int minorVersion, out int patchVersion, out string suffix)
        {
            string unityVersion = Application.unityVersion;

            string[] unityVersionSplit = unityVersion.Split('.');
            majorVersion = Int32.Parse(unityVersionSplit[0]);
            minorVersion = Int32.Parse(unityVersionSplit[1]);
            string patch = unityVersionSplit[2];
            int suffixPos = 0;
            while (char.IsDigit(patch, suffixPos))
                suffixPos++;
            patchVersion = Int32.Parse(patch.Substring(0, suffixPos));
            if (suffixPos >= patch.Length)
                suffix = "";
            else
                suffix = patch.Substring(suffixPos);
    ...
    public class RenderTexturePixelHash
    {
        ...
        public bool ContainsColor32(Color32 color)
        {
            int hash = BitConverter.ToInt32(new byte[] { color.r, color.g, color.b, color.a }, 0);
    ...
    public static class ByteArrayStringHelper
    {

        public static byte[] GetByteArrayFromRawMemory(IntPtr zeroTermCStr)
        {
            List<byte> byteList = new List<byte>();
            byte b = 0;
            int stringLen = 0;
            while ((b = Marshal.ReadByte(zeroTermCStr, stringLen)) != 0)
            {
                byteList.Add(b);
                ++stringLen;
            }
        ...
        // Not safe when called back from native code
        ...
        public static void WriteRawMemoryFromByteArray(IntPtr memoryPtr, byte[] byteArray)
        {
            Marshal.Copy(byteArray, 0, memoryPtr, byteArray.Length); 
            Marshal.WriteByte(memoryPtr, byteArray.Length, 0); // tailing zero
        }
        ...
        public static bool ByteArrayCompare(byte[] lhs, byte[] rhs, int length)
        {
            int arrayLength = lhs.Length > rhs.Length ? rhs.Length : lhs.Length;
        ...
        public static IntPtr CreateRawMemoryFromString(string s)
        {
            IntPtr memoryPtr = Marshal.AllocHGlobal((int)s.Length+1);
            byte[] byteArray = GetByteArrayFromString(s);
            Marshal.Copy(byteArray, 0, memoryPtr, s.Length); 
            Marshal.WriteByte(memoryPtr, s.Length, 0); // tailing zero

            return memoryPtr;
        }

        public static void DestroyRawMemory(IntPtr memoryPtr)
        {
