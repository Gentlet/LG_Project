...
namespace HVR
{
    ...
    public class HvrRender : MonoBehaviour
    {
        ...
        public bool IsCameraDepthRendering {
            get { return m_cameraDepthRendering; }
        }
        ...
        private bool m_cameraDepthRendering = false;
        ...
        public void ForceRedraw()
        {
            ...
            UpdateViewports();
            HvrScene.Player_ResetFrameCounter();
            HvrScene.Player_PrepareRender(GetHvrRenderColorBuffer, GetHvrRenderDepthBuffer);
            if (mode == eMode.standard)
            {
                RenderStandardActors();
        ...
        private void ClearNullActors()
        {
            // Hack to ensure to remove any null HvrActors are not rendered this frame
            // TODO Change how HvrRender works so it's not referencing HvrActors, perhaps a 'Renderable' concept?
            int nullActors = 0;
            ...
            if (nullActors > 0)
            {
                m_renderBundles.RemoveAll(x => x.actor == null);
            }
        }
        ...
        public void WillRender()
        {
            if (m_renderBundles != null)
            {
                ...
                for (int i = 0; i < m_renderBundles.Count; i++)
                {
                    ...
                    {
                        // FIXME: Support VEP on Apple devices
#if UNITY_IPHONE || UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
                        if (bundle.actor.assetInterface.GetCodec() == "VEP")
                        {
                            Debug.LogWarning("VEP codec actor are not supported on macOS and iOS yet.");
                            continue;
                        }
#endif                        
        ...
        private void RenderStandardActors()
        {
            ...
            for (int i = 0; i < renderBundles.Count; i++)
            {
                ...
                if (actor.isActiveAndEnabled &&
                    ...
                    isVisible)
                {
                    // Only add render bundle when the bound is valid, otherwise you might be using the texture used 
                    // last time
                    if (actor.assetInterface.GetBounds().size.sqrMagnitude > 0)
                    {
        ...
        private void _DoRenderStandardActors(List<RenderBundle> renderBundleList, CommandBuffer cmdBuf, CommandBuffer cmdBufComposite, DoRenderStandardActorMaterialPassSelector passSelector, bool isTransparent)
        {
            ...
            for (int i = 0; i < renderBundleList.Count; i++)
            {
                ...
                // IMPORTANT
                // Force HvrActor rendermesh to be updated after rendering the actor
                actor.UpdateRenderMesh();
