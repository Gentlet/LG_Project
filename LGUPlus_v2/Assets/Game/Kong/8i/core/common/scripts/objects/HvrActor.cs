...
namespace HVR
{
    ...
    public class HvrActor : MonoBehaviour
    {
        ...
        public string selectedCodec {
            get 
            {
                return m_selectedStreamingCodec;
            }
            set
            {
                m_selectedStreamingCodec = value;
            }
        }

        public bool pausePlayWhenCaching = false;
...
#if UNITY_EDITOR
        // Intermediate storage for editor
        public string editingData = "";
#endif
        /// <summary>
        ...
        private AssetInterface m_assetInterface = new AssetInterface();

        private ITimestampProvider m_timestampProvider = null;

#if UNITY_EDITOR
        

#if USING_PEEKING_ASSET
        [SerializeField]
        private string[] m_availableCodecs = new string[] {};
        private PeekingAssetInterface m_peekingAssetInterface = new PeekingAssetInterface();
#else
        [SerializeField]
        private readonly string[] m_availableCodecs = new string[] {
            "HVR53",
            "VEP"
        };
        ...
        [SerializeField]
#if UNITY_WEBGL
        private string m_selectedStreamingCodec = "HVR53";
#else        
        private string m_selectedStreamingCodec = "VEP";
#endif
        ...
        private void Update()
        {
            ...
            {
                m_assetInterface.ClearAssetRepresentationDataCache();
                if (timestampProvider != null)
                {
                    if (timestampProvider.ProvidesTimestamp())
                    {
                        m_assetInterface.Update(timestampProvider.GetTimestamp());
                    }
                    else
                    {
                    ...
                    m_assetInterface.assetSource == AssetInterface.AssetSource.RealTime)
                {
                    ...
                    if (pausePlayWhenCaching)
                    {
                        // HACKHACK : currently buffer is hard coded to 2 seconds and caching state is not being set
                        // so add temp check here, should be using assetState.isCaching()
                        float duration = m_assetInterface.GetDuration();
                        float fillRatio = m_assetInterface.GetBufferFillRatio();
                        if ((fillRatio < 0.25) && duration > 2.0 && duration - m_assetInterface.GetActualTime() > 2.0)
                        {
                            if (!timestampProvider.IsCaching())
                                timestampProvider.MarkAsCaching(true);
                        }
                        else {
                            if (timestampProvider.IsCaching())
                                timestampProvider.MarkAsCaching(false);
                        }
                    }
                }
        ...
        private void FirstPush()
        {
            if (timestampProvider != null)
            {
                timestampProvider.Seek(assetSeekTime);

                timestampProvider.SetLooping(assetLoop);
        ...
        private void Init()
        {
            ...
            if (m_renderMethodInterface == null)
            {
                ...
                SetRenderMethodType(m_renderMethodType);
            }
...
#if UNITY_EDITOR
#if USING_PEEKING_ASSET
            if (m_peekingAssetInterface != null)
            {
                m_peekingAssetInterface.Delete();
            }
#endif
#endif            
        ...
        public void SetAssetData(string data, eDataMode dataMode)
        {
            m_data = data;
            m_dataMode = dataMode;
            ...
            {
                AssetInterface theAssetInterface = new AssetInterface();
                theAssetInterface.Create(path);
                theAssetInterface.SetSelectedCodec(selectedCodec);
                SetAssetInterface(theAssetInterface);
#if UNITY_EDITOR
#if USING_PEEKING_ASSET
                m_peekingAssetInterface.Create(path);
#endif
#endif                
            }
        }

        public void SetRenderMethodType(string renderMethodType)
        {
            m_renderMethodType = renderMethodType;

#if UNITY_EDITOR
            // Block this function from running if this object is a prefab
            if (PrefabUtility.GetPrefabType(this) == PrefabType.Prefab)
            {
                Debug.LogWarning("Creating a RenderMethodInterface on a HvrActor prefab is not supported");
                return;
            }
#endif
        ...
        {
#if USING_PEEKING_ASSET
            if (m_peekingAssetInterface.IsPeekingFinished())
            {
                if (m_peekingAssetInterface.HasError() )
                {
                    m_peekingAssetInterface.Repeek();
                }
                else 
                {
                    m_availableCodecs = m_peekingAssetInterface.GetCodecList().ToArray();
        ...
        public int GetCodecIndex(string codec)
        {
            return System.Array.IndexOf(m_availableCodecs, codec);
        }
#endif
        // Actor Interface
        //-------------------------------------------------------------------------

        public void _ForceSeek(float time)
        {
            if (timestampProvider != null && timestampProvider.ProvidesTimestamp())
            {
                timestampProvider.Seek(time);
            }
            else  if (assetInterface != null)
            {
                assetInterface.Seek(time);
            }
        }

        // TODO: remove the external accessbility of AssetInterface
        public void _ReplaceAssetInterface(AssetInterface asset)
        {
            SetAssetInterface(asset);
        }

        private void SetAssetInterface(AssetInterface asset)
        {
#if UNITY_EDITOR
            // Block this function from running if this object is a prefab
            if (PrefabUtility.GetPrefabType(this) == PrefabType.Prefab)
            ...
            else
            {
                ...
                m_assetInterface.SetSelectedCodec(selectedCodec);
        ...
        {
            // If this object is a prefab do not allow it to update
            if (PrefabUtility.GetPrefabType(this) == PrefabType.Prefab)
                return;
