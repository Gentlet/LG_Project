        ...
        static readonly int[] SHADOWMAP_RESOLUTION_FOR_DIRECTIONALLIGHT = new int[]
       {
        ...
        void _DoRenderFromLight()
        {
            ...
            if (lightType == LightType.Directional)
            {
                #region Directional
#if UNITY_2017_1_OR_NEWER                
                if (m_DependentLight.shadows != LightShadows.None &&
                    QualitySettings.shadows != UnityEngine.ShadowQuality.Disable)
                {
                    ...
                    if (cam != null)
                    {
                        ...
                        HvrRender hvrRender = cam.GetComponent<HvrRender>();
                        if (hvrRender != null && hvrRender.IsCameraDepthRendering) // Only when HVR depth renders into camera depth texture, the following shadow calculation is valid
                        {
